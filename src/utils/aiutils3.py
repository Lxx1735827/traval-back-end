...
def parser_Message(message, file_name):
    data = json.loads(message)
    code = data['header']['code']
    if code != 0:
        raise HTTPException(status_code=400, detail="图片生成失败")
    else:
        text = data["payload"]["choices"]["text"]
        imageContent = text[0]
        imageBase = imageContent["content"]
        base64_to_image(imageBase,file_name)


def create_picture(content, name):
    APPID = '223a24c2'
    APISecret = 'ZjhlZDQyMDU3Y2NhYjMwYmExZGRmYmMy'
    APIKEY = 'e2f620fdc9d52be5e0f792d4e1755416'
    res = main(content, appid=APPID, apikey=APIKEY, apisecret=APISecret)
    parser_Message(res, name)
    return


if __name__ == '__main__':
    create_picture("盗墓笔记", "盗墓笔记.jpg")
